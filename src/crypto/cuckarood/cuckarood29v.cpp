...
#include "int-util.h"
#include "cuckarood29v.h"


void Cuckarood29V::setsipkeys(const unsigned char *keybuf,siphash_keys *keys) {
...
void Cuckarood29V::setheader(const unsigned char *header, const uint32_t headerlen, siphash_keys *keys) {
...
uint64_t  Cuckarood29V::rotl(uint64_t x, uint64_t b) {
	return (x << b) | (x >> (64 - b));
}
void Cuckarood29V::sip_round(void) {
...
void  Cuckarood29V::hash24(const uint64_t nonce) {
	v3 ^= nonce;
	sip_round(); sip_round();
	v0 ^= nonce;
	v2 ^= 0xff;
...
uint64_t Cuckarood29V::xor_lanes(void) {
	return (v0 ^ v1) ^ (v2  ^ v3);
}
uint64_t Cuckarood29V::sipblock(siphash_keys *keys, const uint32_t edge,uint64_t  *buf) {
	v0=keys->k0;
	v1=keys->k1;
	v2=keys->k2;
	v3=keys->k3;
...
int Cuckarood29V::verify(uint32_t edges[PROOFSIZE], siphash_keys *keys) {
...
int Cuckarood29V::hash(const void* in, size_t len, uint32_t nonce, uint32_t *edges,void* out)
{
